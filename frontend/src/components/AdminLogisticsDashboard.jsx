...
import { useLedger } from '../context/LedgerContext'; // NEW: Import Ledger Context
...
const AdminLogisticsDashboard = () => {
  ...
  const { addBlock } = useLedger(); // NEW: Initialize Ledger hook
  ...
  const updateStatus = async (orderId, status) => {
    try {
      ...
      // NEW: Add the Transit Node Block to the Ledger!
      addBlock(
        `NODE_${status}`, 
        `Order ${orderId} cryptographic transit location updated to ${status} at Warehouse Alpha.`
      );
